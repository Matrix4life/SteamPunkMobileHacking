...
setVirusScans({});
  ...
  useEffect(() => {
    ...
    const retaliationTick = setInterval(() => {
      ...
      rivals
        .filter(r => r.status === 'active' && !r.recruited)
        .forEach(rival => {
    ...
    return (
      <div style={{background:'#08080c',minHeight:'100vh',color:'#78dce8',fontFamily:'monospace',padding:'20px',maxWidth:720,margin:'0 auto'}}>
        <div style={{borderBottom:'1px solid #2d2a2e',paddingBottom:10,marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <button onClick={() => setScreen('game')} style={{background:'none',border:'1px solid #2d2a2e',color:'#78dce8',padding:'4px 12px',cursor:'pointer',fontFamily:'monospace'}}>X CLOSE</button>
        </div>
        {c ? (
          <div style={{background:'#0f0f14',border:'1px solid #ffd866',padding:'12px 16px',marginBottom:20,fontSize:13}}>
            ...
            {(c.objectives || []).map((o, i) => (
              <div key={i} style={{color:'#78dce8',marginTop:4}}>{objLine(o, i)}</div>
            ))}
            <div style={{color:'#ff6188',marginTop:6}}>HEAT CAP: {c.heatCap}% | REWARD: {c.reward ? c.reward.toLocaleString() : 0} | REP: +{c.repReward}</div>
            {(c.forbidden_tools || []).length > 0 && (
              <div style={{color:'#fc9867',marginTop:4}}>FORBIDDEN: {(c.forbidden_tools || []).join(', ')}</div>
            )}
          </div>
        ) : (
          <div style={{color:'#2d2a2e',border:'1px solid #2d2a2e',padding:'10px 16px',marginBottom:20,fontSize:13}}>No active contract.</div>
        )}
        ...
        <textarea
          ...
          style={{width:'100%',height:320,background:'#0a0a10',color:'#a9dc76',border:'1px solid #2d2a2e',fontFamily:'monospace',fontSize:13,padding:12,resize:'vertical',boxSizing:'border-box',outline:'none',lineHeight:1.6}}
        />
        <div style={{display:'flex',gap:10,marginTop:12}}>
          <button onClick={() => { saveGame(operator); setTerminal(prev => [...prev, {type:'out',text:'[+] Notes saved.',isNew:true}]); }} style={{background:'#a9dc76',color:'#08080c',border:'none',padding:'8px 20px',fontFamily:'monospace',fontWeight:'bold',cursor:'pointer'}}>SAVE NOTES</button>
          <button onClick={() => setPlayerNotes('')} style={{background:'none',border:'1px solid #ff6188',color:'#ff6188',padding:'8px 20px',fontFamily:'monospace',cursor:'pointer'}}>CLEAR</button>
        </div>
      </div>
    );
  }
