...
const SLOTS = Object.keys(SLOT_LAYOUT);
...
const ANIM_CSS = `
  ...
  @keyframes slotBreath   { 0%,100%{fill-opacity:0.03} 50%{fill-opacity:0.09} }
  @keyframes slotGlowPulse{ 0%,100%{stroke-opacity:0.45} 50%{stroke-opacity:0.8} }
  ...
  .slot-breath     { animation: slotBreath 2.8s ease-in-out infinite; }
  .slot-glow       { animation: slotGlowPulse 2.2s ease-in-out infinite; }
...
function Rotator({ cx, cy, active, reverse, slow, children }) {
  if (!active && !slow) return <g opacity={0.5}>{children}</g>;
...
function SlotIcon({ slot, x, y, tier, color, isProcessing, animateAmbient }) {
  const c = tier > 0 ? color : '#2a3545';
  const dim = tier > 0 ? 1.0 : 0.3;
  const cx = x;
  const cy = y;
  const active = tier > 0 && isProcessing;
  const slowSpin = tier > 0 && animateAmbient;
      ...
      return (
        <g opacity={dim}>
          ...
          <g clipPath={`url(#${clipId})`}>
            <Rotator cx={cx-4} cy={cy} active={active} slow={slowSpin}>
            ...
            <Rotator cx={cx+5} cy={cy} active={active} reverse slow={slowSpin}>
    ...
    case 'PSU':
      return (
        <g opacity={dim}>
          ...
          {tier > 0 && (
            <Rotator cx={cx+6} cy={cy+4} active={false} slow={slowSpin}>
              {[0,90,180,270].map(a => {
                const rad = a * Math.PI / 180;
    ...
    case 'COOL': {
      ...
      return (
        <g opacity={dim}>
          ...
          <g clipPath={`url(#${clipId})`}>
            <Rotator cx={cx} cy={cy} active={active} slow={slowSpin}>
...
function Slot({ slot, pos, tier, selected, onClick, isSelectedProcessing, animateAmbient, rgbPhase }) {
    ...
    <g onClick={onClick} style={{ cursor: 'pointer' }}>
      {tier > 0 && (
        <rect
          ...
          className={animateAmbient && !selected ? 'slot-glow' : ''}
        />
      )}
      ...
      <rect
        ...
        className={animateAmbient && tier > 0 ? 'slot-breath' : ''}
      />
      ...
      <SlotIcon
        ...
        animateAmbient={animateAmbient}
      />

      {isSelectedProcessing && tier > 0 && (
        <rect
          x={pos.x+6}
          y={pos.y+pos.h-5}
          width={pos.w-12}
...
function DetailPanel({ slot, partId, heat, cpuPct, gpuPct, isProcessing, animateAmbient }) {
  ...
  return (
    <g transform="translate(302 30)">
      <rect
        ...
        className={animateAmbient && tier > 0 ? 'slot-glow' : ''}
      ...
      <g transform="translate(87.5 86) scale(2.8)">
        <SlotIcon slot={slot} x={0} y={0} tier={tier} color={glow.stroke} isProcessing={isProcessing} animateAmbient={animateAmbient} />
      </g>
      ...
      <text x={14} y={208} fill={statusColor} fontSize="8" className={heat >= 78 ? 'blink-warn' : ''} style={{ letterSpacing:'1.5px' }}>
        {statusText}
      </text>
...
function CollapsedView({ tiers, heat, isProcessing, animateAmbient }) {
  const statusColor = heat >= 78 ? COLORS.danger : heat >= 45 ? COLORS.warning : COLORS.secondary;
          ...
          return (
            <div
              ...
              style={{
                ...
                animation: animateAmbient && tier > 0 ? 'slotBreath 2.8s ease-in-out infinite' : 'none',
              }}
            >
              {slot.slice(0,2)}
            </div>
...
export default function RigDisplay({
  ...
  const activeEffects = isProcessing && !reducedMotion;
  const ambientEffects = !isProcessing && !reducedMotion;
  const rgbEnabled = hasRGB && !reducedMotion;

  useEffect(() => {
    if (!rgbEnabled) return undefined;
  ...
  return (
    <div
      style={{
        ...
        position: 'relative',
        background: isHot
          ? 'radial-gradient(circle at center, rgba(255,80,80,0.14), rgba(10,15,20,1) 68%)'
          : COLORS.bgDark,
        overflow: 'hidden',
        borderRadius: '8px',
        ...
        boxShadow: isHot
          ? `0 0 18px ${COLORS.danger}22, inset 0 0 24px rgba(255,80,80,0.08)`
    ...
    >
      ...
      >
        ...
        <button
          ...
          aria-label={expanded ? 'Collapse rig display' : 'Expand rig display'}
      ...
      {expanded ? (
        <svg width="480" height="260" viewBox="0 0 480 260">
          <g
            ...
            style={{
              filter: isHot ? 'drop-shadow(0 0 10px rgba(255,80,80,0.18))' : 'none',
            }}
          >
            <rect
              x={4}
              ...
              stroke={rgbEnabled ? `hsla(${rgbPhase % 360}, 60%, 50%, 0.3)` : 'rgba(120,220,232,0.1)'}
              strokeWidth="1"
            />
            ...
            {Object.entries(SLOT_LAYOUT).map(([slot, pos]) => (
              <Slot
                ...
                isSelectedProcessing={activeEffects && selected === slot}
                animateAmbient={ambientEffects}
                rgbPhase={rgbPhase}
